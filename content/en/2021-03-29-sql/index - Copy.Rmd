...
Some information contains from Datacamp, you can use [my refer](https://www.datacamp.com/join-me/MjY2NTU1) to get started and learn new technique together.
...
OVER() cause allow an aggregate function to a dataset and similar to the subquery and run faster than the subqueries. This is difference from the aggregate function with group by. The major difference is that __group by__ usually will change the row number. And use the over(partition by xx) does not change the row number. 
...
1. the main difference between __having__ and __where__. The having used on with the group by, so it something uses like having avg(xx) > xx. There where is usually more general as a filter/condition. So the sequence of writing the three clause is 1. where. 2. group by 3. having. 

2. function used in the select clause does not related to __where__, and __having__
